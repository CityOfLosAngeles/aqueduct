...
-- create new table for postgis geometry
CREATE TABLE waze.geoTable(
  id BIGSERIAL PRIMARY KEY,
  alert_id BIGINT REFERENCES waze.alerts (pid), -- null if not alert geometry
  jam_id BIGINT REFERENCES waze.jams (pid),
  irregularity_id BIGINT REFERENCES waze.irregularities (pid)
);

-- add geometry column (schema,table,colName,srid,type,dimension)(4326=wgs84 projection)
