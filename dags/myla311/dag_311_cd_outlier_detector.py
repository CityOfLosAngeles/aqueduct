...
def detect_outliers(filename, **kwargs):
    ...
    df["year_week"] = (
        df["cal_year"].map(str)
        + "."
        + df["cal_week_number"].apply(lambda x: "0" + str(x) if x < 10 else x).map(str)
    )
    ...
    df["process_time"] = (df["closed_datetime"] - df["created_datetime"]).apply(
        lambda x: x.total_seconds() / 3600
    )
    df_process_time = df.loc[
        :, ["cd", "srnumber", "created_date", "request_type", "process_time"]
    ]
    ...
    first_day_of_last_yr_week = same_day_of_last_year - timedelta(
        days=delta_to_first_day_of_that_week
    )
    ...
    for cd in cd_dict.keys():
        ...
        for x in cols:
            ...
            if (
                df_last_week[x]
                > df_last_whole_yr[x].mean() + 2 * df_last_whole_yr[x].std()
            ):
                ...
                flag = True
            if flag:
                filename = make_save_graph(
                    df_last_whole_yr, cd, x, "Weekly Indiv Req Type Outlier"
                )
        ...
        if flag:
            filename = make_save_graph(
                df_last_whole_yr, cd, col, "Weekly Total Outlier"
            )
            alert[col].append(filename)

        # Get Diff of weekly total outliers
        pd.options.mode.chained_assignment = (
            None  # turn off Value to be set on a copy of a slice warning
        )
        ...
        for req_type in cols:
            ...
            df_temp1 = df_proc_time_last_week[
                df_proc_time_last_week.request_type == req_type
            ]
            for index, row in df_temp1.iterrows():
                flag = False

                if row["process_time"] > high_bound:
                    alert["srnumber: " + row["srnumber"]] = [
                        HIGH_PROCESS_TIME_OUTLIER
                        + ", "
                        + row["request_type"]
                        + ", "
                        + str(row["process_time"])
                        + " hrs"
                    ]
                    flag = True
                elif row["process_time"] < low_bound:
                    alert["srnumber: " + row["srnumber"]] = [
                        LOW_PROCESS_TIME_OUTLIER
                        + ", "
                        + row["request_type"]
                        + ", "
                        + str(row["process_time"])
                    ...
                    flag = True
                if flag:
                    filename = make_save_boxplot(
                        df_temp["process_time"],
                        cd,
                        row["process_time"],
                        req_type + " SRNUMBER_" + row["srnumber"],
                    )
        ...
        for key, value in alert.items():
            ...
            if os.path.exists(prefix + file):
                ...
                print(
                    "The file "
                    + file
                    + " does not exist. Skipping to the next graph png"
                )
