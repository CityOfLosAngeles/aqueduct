...
def detect_outliers(filename, **kwargs):
    ...
    for x in cols:
        ...
        if df_last_week[x] > df_last_whole_yr[x].mean() + 2 * df_last_whole_yr[x].std():
            ...
            df_last_week[x] < df_last_whole_yr[x].mean() - 2 * df_last_whole_yr[x].std()
        ):
            alert[x] = [INDIV_LOW_OUTLIER]
            flag = True
        if flag:
            filename = make_save_graph(
                df_last_whole_yr, x, "Weekly Indiv Req Type Outlier"
            )
...
def sendemail(to_addr_list, cc_addr_list, subject, message, **kwargs):
    ...
    if bool(alert):
        ...
        send_email(to_addr_list, subject, html_content, files=files, cc=cc_addr_list)
...
def remove_graph_png(**kwargs):
    ...
    for key, value in alert.items():
        ...
        if os.path.exists(prefix + file):
            ...
            print(
                "The file " + file + " does not exist. Skipping to the next graph png"
            )
