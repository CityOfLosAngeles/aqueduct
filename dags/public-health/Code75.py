...
def update_code_75(**kwargs):
    ...
    gis = arcgis.GIS("http://lahub.maps.arcgis.com/", arcconnection.login, arcconnection.password)
    ...
    for schema in schemas:
        if schema == "public-health":
            if "Code75s" in inspector.get_table_names(schema=schema):
                engine.connect().execute('DELETE FROM "public-health".Code75s WHERE "OBJECTID" IN (%s)' % object_id_list)
                break
...
t1 = PythonOperator(
    ...
    op_kwargs={
        "arcfeatureid": "6b757be10dd04edb9c139894805425a1",
    },
    dag=dag,
)