...
from socrata_helpers import _read_paginated, _store_and_attach_dataset, write_and_attach_jsonvalue, _store_and_attach_metadata
...
def main(dataset_id, table_name, database, socrata_username, socrata_password):
    ...
    socrata_client = Socrata("data.lacity.org",
                                           None,
    ...
    if dataset.empty:
        ...
        if table_name:
            ...
            LOG.info(
                f"Storing data in table {table_name} on database {database}"
            )
            print("writing table")
    ...
    metadata_file_name = f"{dataset_id}_metadata_{datetime.now().strftime('%Y-%m-%d')}.json"

    metadata_paths = {"Proposed access level":
...
if __name__ == '__main__':
    ...
    if "table_name" in list(os.environ.keys()) and "database" in list(os.environ.keys()):
